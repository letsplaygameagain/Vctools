...
• `{i}addauth` `tambahkan auth. `
  `{i}addauth admins` - `izinkan hanya Admin untuk Menggunakan Vc`
...
• `{i}vcaccess <id/username/reply to msg>`
    Berikan akses Bot Obrolan Suara.

• `{i}rmvcaccess <id/username/reply to msg>`
    Hapus akses Bot Obrolan Suara.

• `{i}listvcaccess`
    Dapatkan Daftar Orang yang memiliki akses vc.
"""
...
async def _(e):
    xx = await e.eor("`Menolak untuk mengakses fitur Obrolan Suara...`")
...
async def _(e):
    xx = await e.eor("`Menyetujui untuk mengakses fitur Obrolan Suara...`")
    input = e.pattern_match.group(1).strip()
    if e.reply_to_msg_id:
        userid = (await e.get_reply_message()).sender_id
        name = (await e.client.get_entity(userid)).first_name
